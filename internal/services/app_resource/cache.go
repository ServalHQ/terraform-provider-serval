		...
		for {
			...
			if err != nil {
				if cache.IsServerError(err) {
					tflog.Warn(ctx, fmt.Sprintf("prefetch: skipping app_resources for team %s due to server error: %s", teamID, err))
					break
				}
				return apiCalls, err
			}
...
func TryRead(id string) (*AppResourceModel, bool, error) {
