...
func Prefetch(ctx context.Context, client *serval.Client, teamIDs []string) (int, error) {
	...
	for _, teamID := range teamIDs {
		...
		for {
			params := serval.WorkflowApprovalProcedureListByTeamParams{
			...
			_, err := client.Workflows.ApprovalProcedures.ListByTeam(ctx, params,
			...
			if err != nil {
				if cache.IsServerError(err) {
					tflog.Warn(ctx, fmt.Sprintf("prefetch: skipping workflow_approval_procedures for team %s due to server error: %s", teamID, err))
					break
				}
				return apiCalls, err
			}
