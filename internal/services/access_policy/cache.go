		...
		for {
			...
			if err != nil {
				if cache.IsServerError(err) {
					tflog.Warn(ctx, fmt.Sprintf("prefetch: skipping access_policies for team %s due to server error: %s", teamID, err))
					break
				}
				return apiCalls, err
			}
...
func TryRead(id string) (*AccessPolicyModel, bool, error) {
